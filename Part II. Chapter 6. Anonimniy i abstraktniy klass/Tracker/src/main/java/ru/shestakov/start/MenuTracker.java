...
class UpdateItem extends BaseAction {
    ...
    public void execute(Input input, Tracker tracker) {
        ...
        if(tracker.findById(id) != null) {
            ...
            long create = 0;
            try {
                create = Long.parseLong(input.ask("Please, enter the task`s date: "));
...
public class MenuTracker {
    ...
    private class FindItemByCreate extends BaseAction {
        ...
        public void execute(Input input, Tracker tracker) {
            String createS = input.ask("Please, enter the task`s date to find: ");
            ...
            if(createS.equals("b")) {
                ...
                long createL = 0;
                try {
                    createL = Long.parseLong(createS);
                } catch (NumberFormatException nfe) {
                    System.out.println("Incorrect value of task`s date. Please, enter correct date again.");
                    execute(input,tracker);
                }

                Filter filter = new FilterByCreate(createL);
